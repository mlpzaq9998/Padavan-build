...
#else
#define MTD_STORE_PART_SIZE		0x00020000	/*  128K */
#endif
...
#endif
	/* calc "RootFS" size */
	rt2880_partitions[4].size = flash_size - (MTD_KERNEL_PART_OFFSET + rt2880_partitions[MTD_STORE_PART_IDX].size + rt2880_partitions[3].size + MTD_Config_PART_SIZE + MTD_FACTORY_PART_SIZE);
#else
	/* ROOTFS_IN_RAM */
	rt2880_partitions[3].size = flash_size - (MTD_KERNEL_PART_OFFSET + rt2880_partitions[MTD_STORE_PART_IDX].size + MTD_Config_PART_SIZE + MTD_FACTORY_PART_SIZE);
#endif

	/* calc "Firmware_Stub" size (allow crossing over "Storage" for <= 8 MB flash) */
	rt2880_partitions[MTD_FWSTUB_PART_IDX].size = flash_size - MTD_KERNEL_PART_OFFSET - MTD_Config_PART_SIZE - MTD_FACTORY_PART_SIZE;
	if (flash_size > 0x00800000)
